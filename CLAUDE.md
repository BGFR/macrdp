...
- **Server-side scaling forces full-frame updates.** Passing `--width N --height M` when N×M ≠ the Mac's native size makes SCK scale internally and emit dirty-rects in *source* (native) coords — those rects no longer line up with the output buffer, and clients that strictly respect tile coordinates (mstsc/RemoteFx in particular) render a black canvas with only the cursor sprite. `capture.rs` detects the size mismatch up front and disables dirty-rect emission, sending a full BitmapUpdate every tick; the upstream encoder's framebuffer diff still keeps unchanged-region cost low when SCK happens to return exactly the configured size. The user-visible cost is higher bandwidth at non-native sizes — prefer native if you can.
