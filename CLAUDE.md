...
src/file_promise_lazy.rs  Windows→Mac LAZY paste (--lazy-paste): pre-
...
vendor/ironrdp-egfx/      Local fork of ironrdp-egfx (same upstream rev as the
                          ...
                          ONE divergence: CapabilitySet::decode tolerates unknown
                          capability versions (keeps them as Unknown) instead of
                          erroring fatally — without this, Windows App for Mac
                          can't connect with --enable-h264 (see quirk note).
                          Submitted upstream; drop this vendor dir once it lands
                          in a released ironrdp-egfx. (A second divergence,
                          create_surface_with_id for fresh per-session surface
...
--lazy-paste              # Windows→Mac file paste streams on Cmd-V (NSFilePresenter)
                          #   instead of eager download; shows native "Preparing to paste"
                          #   progress, lower chunk parallelism so RDP stays responsive
