...
Not yet implemented: multi-monitor (client-side multi-display), printer redirection. (Non-US keyboard layouts work, **auto-detected from the client by default** — `--keyboard-layout` overrides, `none` disables.) **Drive redirection (RDPDR) is in progress** behind `--enable-drive-redirection` (opt-in, read-only): Phase 1a — MS-RDPEFS handshake + drive discovery (verified on FreeRDP **and** mstsc); Phase 1b — device I/O, the server reads a file off the client's drive via `RdpdrHandle::read_file` (create→read→close, matched by a completion-id router; verified on FreeRDP). Remaining: directory listing + the macOS Finder temp-folder surface. See the vendored `ironrdp-rdpdr` / `ironrdp-server` divergence logs.
