...
- Drive keyboard and mouse, including modifier keys (per-side L/R tracking with NX_DEVICE bits, Caps Lock as a toggle, MS-RDPBCGR Synchronize lock-state reconciliation), mouse buttons, and wheel. Keyboard input defaults to US ANSI (positional scancode→macOS keycode), so the Mac's active input source decides the character — but **`--keyboard-layout <name|KLID|input-source-id>` serves non-US layouts** (e.g. `french`, `de`, `0x040C`, `com.apple.keylayout.French`) by translating each typing key against that layout via Carbon `UCKeyTranslate` and posting the resulting character as a Unicode string, **without changing the Mac's own input source**. Cmd/Ctrl combos stay on the keycode path so shortcuts work; dead keys (´+e→é) compose via UCKeyTranslate's persistent state. See `src/keyboard_layout.rs` and the keyboard-layout quirk note.
...
Not yet implemented: multi-monitor (client-side multi-display), drive/printer redirection. (Non-US keyboard layouts are supported via `--keyboard-layout`; auto-detecting the layout from the client's GCC Client Core Data — so the flag isn't needed — is a possible follow-up.)
...
--keyboard-layout SPEC    # Interpret the client's keystrokes with a non-US
                          #   layout (name like `french`/`de`/`azerty`, a Windows
                          #   KLID like 0x040C, or a macOS input-source id). Keys
                          #   are translated via UCKeyTranslate and posted as
                          #   Unicode; the Mac's own input source is untouched.
                          #   The layout must be installed on the Mac. macOS-only.
