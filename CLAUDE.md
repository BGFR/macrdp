...
Not yet implemented: multi-monitor (client-side multi-display), printer redirection. (Non-US keyboard layouts work, **auto-detected from the client by default** — `--keyboard-layout` overrides, `none` disables.) **Drive redirection (RDPDR)** behind `--enable-drive-redirection` (opt-in, **read-only**): the connecting client redirects its local drive and the Mac mounts it as a Finder folder. Phase 1a — MS-RDPEFS handshake + drive discovery (verified FreeRDP **and** mstsc); 1b — device I/O via `RdpdrHandle` (`read_file`, `list_dir`, matched by a completion-id router); 1c — the macOS Finder surface (`src/rdpdr/surface.rs`): on connect the client's drive root is mirrored into a temp folder (subdirs as empty folders, files as pre-sized placeholders each backed by an `NSFilePresenter`), opened in Finder, and streamed on demand via `read_to_file` when Finder reads a file — reusing `runloop_thread`; cleaned up on disconnect (Surface::Drop). **Top-level only** (subdir contents + writes are Phase 2). See the vendored `ironrdp-rdpdr` / `ironrdp-server` divergence logs and the RDPDR quirk notes.
