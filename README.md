...
v0 — daily-driver usable on a trusted LAN, and usable **over the internet** (VPN / ZeroTier / high-latency links, including mobile). **Latest release: [v0.8.39](https://github.com/clintcan/macrdp/releases/latest)** — *the smooth-resize release*: live client-driven resize on the **headless** path (virtual display + capture/detach-primary) is now smooth and consistent — resizing/maximizing (including dragging between monitors of different resolutions) no longer flickers, cuts audio, loses the Dock, or strands your windows. Root cause was the vd's main-display placement never being persisted, so every re-mode snapped the arrangement back; plus a reactivation-aware overlay watcher and an automatic post-resize window gather. The default runtime path is unchanged. Builds on v0.8.38 (the **`Ctrl+Alt+Shift+R`** *A/V resync hotkey* for idle-stale sessions), v0.8.37 (*live resize* via MS-RDPEDISP + the *USB webcam stall watchdog*), and v0.8.36 (the *fork-workers removal* + camera-redirection Phase-0 groundwork).
