...
v0 — daily-driver usable on a trusted LAN, and usable **over the internet** (VPN / ZeroTier / high-latency links, including mobile). **Latest release: [v0.8.40](https://github.com/clintcan/macrdp/releases/latest)** — *the headless-laptop release*: three fixes for daily headless-laptop use plus a which-client audit signal. Opt-in **`--restore-windows-on-disconnect`** makes your windows follow you — swept back onto the built-in screen when you disconnect (Mac usable locally), auto-gathered onto the virtual display when you reconnect. The **Dock no longer disappears on disconnect** (the v0.8.39 main-display persistence, now applied to the restore path too). And **`--capture-primary` blanking survives a live resize** — a re-mode reset the physical gamma and left the desktop showing; it's now re-asserted (a brief ~250 ms flash during the re-mode itself is a documented gamma-blanking limitation). Plus a new **`event="fingerprint"`** audit record naming which client connected (mstsc vs FreeRDP vs Thincast). The default runtime path is unchanged. Builds on v0.8.39 (the *smooth-resize release*), v0.8.38 (the **`Ctrl+Alt+Shift+R`** *A/V resync hotkey*), and v0.8.37 (*live resize* via MS-RDPEDISP + the *USB webcam stall watchdog*).
