...
v0 — daily-driver usable on a trusted LAN, and usable **over the internet** (VPN / ZeroTier / high-latency links, including mobile). **Latest release: [v0.8.37](https://github.com/clintcan/macrdp/releases/latest)** — *live resize + the webcam stall watchdog*: the session now **resizes live when the client drags its window** (MS-RDPEDISP — verified on Windows App / FreeRDP, a clean no-op on mstsc which doesn't emit the resize PDU; contributed by Anton Mostovoy), and a **redirected USB webcam that freezes now recovers on its own** instead of staying stuck (a read-ahead stall watchdog that re-COMMITs the pipe when the client's camera stalls host-side). Both are opt-in paths; the default runtime path is unchanged. Builds on v0.8.36 (the *fork-workers removal* + camera-redirection Phase-0 groundwork), v0.8.35 (the *USB read-ahead gate fix*), v0.8.34 (the *gamepad-input fix*), and v0.8.33 (the *audit-forwarding release* — an opt-in **structured JSON audit stream** (`--audit-file`) for **SIEM/SOC** forwarding).
