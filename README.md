...
v0 — daily-driver usable on a trusted LAN, and usable **over the internet** (VPN / ZeroTier / high-latency links, including mobile). **Latest release: [v0.8.35](https://github.com/clintcan/macrdp/releases/latest)** — the *USB read-ahead gate fix*: a one-fix point release over v0.8.34 in the opt-in **USB redirection** feature. v0.8.34 fixed the redirected HID gamepad but broke the bulk **webcam** — it scoped the read-ahead engine by the endpoint's *declared* transfer type, and a UVC video endpoint is often reported over the wire as non-bulk, so read-ahead never engaged. Read-ahead is now gated on the transfer's **read length** (a reliable physical signal) instead, so the gamepad's tiny interrupt polls stay serial while the webcam's large streaming reads engage as they did in v0.8.33. Builds on v0.8.34 (the *gamepad-input fix*), v0.8.33 (the *audit-forwarding release* — an opt-in **structured JSON audit stream** (`--audit-file`) for **SIEM/SOC** forwarding, an explicit **`auth` login-verdict event**, and log-injection hardening) and v0.8.32 (the *security-hardening release* — fuzzed the network-facing decoders, `--max-client-size` cap, scheduled `cargo-deny`).
