...
v0 — daily-driver usable on a trusted LAN, and usable **over the internet** (VPN / ZeroTier / high-latency links, including mobile). **Latest release: [v0.8.30](https://github.com/clintcan/macrdp/releases/latest)** — the *webcam release*: a **bulk USB webcam redirected over FreeRDP now streams live video** into the Mac session (`--enable-usb-redirection`, entitled build) — as far as is known a first for any open-source RDP *server*. A bulk-IN read-ahead engine keeps several USB reads in flight to the client at once, restoring the device read-depth macOS needs to assemble frames; it engages only for large streaming reads, so mass storage (byte-exact) and the redirected gamepad are unaffected. Builds on v0.8.29 (the stability release — clipboard-UAF crash, scroll-wheel runaway, Xbox Guide-button session-kill, dev-signing TCC grants).
