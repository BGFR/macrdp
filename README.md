...
v0 — daily-driver usable on a trusted LAN, and usable **over the internet** (VPN / ZeroTier / high-latency links, including mobile). **Latest release: [v0.8.36](https://github.com/clintcan/macrdp/releases/latest)** — the *fork-workers removal*: drops the experimental `--fork-workers` per-connection process model, so single-process is now the only model. It existed to dodge mstsc's H.264 reconnect-blank by serving each connection from a fresh process, but the server has **self-healed that blank in place since v0.8.27** (a bare core deactivation–reactivation + the auto-reconnect cookie), and the process model turned out to be a *net-negative* for interactive mstsc. The default runtime path is unchanged. Also lands opt-in, default-off **camera-redirection groundwork** (`--enable-camera-redirection`, an MS-RDPECAM Phase-0 protocol gate — no capture yet). Builds on v0.8.35 (the *USB read-ahead gate fix*), v0.8.34 (the *gamepad-input fix*), v0.8.33 (the *audit-forwarding release* — an opt-in **structured JSON audit stream** (`--audit-file`) for **SIEM/SOC** forwarding), and v0.8.32 (the *security-hardening release* — fuzzed decoders, `--max-client-size` cap, scheduled `cargo-deny`).
