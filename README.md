...
v0 — daily-driver usable on a trusted LAN, and usable **over the internet** (VPN / ZeroTier / high-latency links, including mobile). **Latest release: [v0.9.0](https://github.com/clintcan/macrdp/releases/latest)** — *the webcam release*: a webcam redirected from the client now presents as a **real macOS camera**. Turn on **`--enable-camera-redirection`**, tick *Video capturing devices* in the client, and **"macrdp Camera"** shows the client's live webcam in Photo Booth / Zoom / FaceTime / Teams. As far as is known the **first known open-source RDP _server_ to present a client-redirected webcam as a native OS camera** — and the path that actually works for **mstsc**, which routes webcams over MS-RDPECAM rather than raw USB. The pipeline is H.264 over the RDCamera channel → VideoToolbox decode → a CoreMediaIO Camera system extension, zero-copy via IOSurface; live-verified at 1080p/~30 fps with zero dropped frames. Opt-in and **default OFF — the default runtime path is byte-identical when off** (presenting the camera needs the signed/notarized build + a one-time extension activation; see [docs/camera-extension-setup.md](docs/camera-extension-setup.md)). Builds on v0.8.40 (the *headless-laptop release*), v0.8.39 (the *smooth-resize release*), and v0.8.38 (the **`Ctrl+Alt+Shift+R`** *A/V resync hotkey*).
...
| **[vs. other OSS RDP servers](docs/oss-rdp-server-comparison.md)** | The evidence behind every "first" claim in these docs — verified adversarially against FreeRDP/xrdp/ogon/gnome-remote-desktop and re-checked in the source. Includes what macrdp is **not** first at, the other native macOS RDP servers that predate it, and how to re-verify when upstreams move. |
