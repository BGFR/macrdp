...
v0 — daily-driver usable on a trusted LAN, and usable **over the internet** (VPN / ZeroTier / high-latency links, including mobile). **Latest release: [v0.8.34](https://github.com/clintcan/macrdp/releases/latest)** — the *gamepad-input fix*: a one-fix point release repairing a regression in the opt-in **USB redirection** feature, where a redirected **HID gamepad enumerated but its buttons did nothing** (broken since v0.8.30). The bulk-IN read-ahead engine added for webcam streaming was matching *interrupt* endpoints as well as bulk ones; read-ahead is now gated on the endpoint's real transfer type, so mass storage and the bulk webcam are unchanged while HID input works again. Builds on v0.8.33 (the *audit-forwarding release* — an opt-in **structured JSON audit stream** (`--audit-file`) for **SIEM/SOC** forwarding, an explicit **`auth` login-verdict event**, and log-injection hardening) and v0.8.32 (the *security-hardening release* — fuzzed the network-facing decoders, `--max-client-size` cap, scheduled `cargo-deny`).
