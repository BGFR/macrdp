...
v0 — daily-driver usable on a trusted LAN, and usable **over the internet** (VPN / ZeroTier / high-latency links, including mobile). **Latest release: [v0.8.32](https://github.com/clintcan/macrdp/releases/latest)** — the *security-hardening release*: fuzzed the network-facing protocol decoders (**found + fixed 3 real panics** on truncated USB-redirection PDUs), added an operator `--max-client-size` cap (closes a post-auth framebuffer-allocation vector), bounded the smart-card loopback bridge, and added scheduled `cargo-deny` dependency-vulnerability scanning — plus `--alt-backtick-switch` (Option+` window cycle) and config-file access to the blank-recovery tunables. No change to the default runtime path. Builds on v0.8.31 (the *gamepad-resilience release* — a redirected USB interrupt-IN endpoint no longer stalls when the client fails a single read).
