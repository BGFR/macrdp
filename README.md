...
v0 — daily-driver usable on a trusted LAN, and usable **over the internet** (VPN / ZeroTier / high-latency links, including mobile). **Latest release: [v0.9.1](https://github.com/clintcan/macrdp/releases/latest)** — *the lockable-headless release*: a new opt-in headless blanking mode, **`--shield-primary`**, covers the physical panel with an opaque black window (via a helper process) instead of capturing it — so unlike `--capture-primary`, **the Mac can still be locked** (and there's no ~250 ms resize flash). Opt-in and default OFF; capture/detach are unchanged. Also lands client-resolution auto-adopt on the `--virtual-display` path, a blank-recovery refinement, a majority-area `Ctrl+Alt+G` window-gather, and a `--detach-primary` launchd-restart stopgap for the macOS-26 panel-re-enable bug. Builds on **v0.9.0** (*the webcam release*: a client webcam presents as a **real macOS camera** via `--enable-camera-redirection` — as far as is known the first known open-source RDP _server_ to do so; H.264 over MS-RDPECAM → VideoToolbox decode → a CoreMediaIO Camera system extension, live-verified at 1080p/~30 fps), v0.8.40 (the *headless-laptop release*), and v0.8.39 (the *smooth-resize release*).
