...
v0 — daily-driver usable on a trusted LAN, and usable **over the internet** (VPN / ZeroTier / high-latency links, including mobile). **Latest release: [v0.8.31](https://github.com/clintcan/macrdp/releases/latest)** — the *gamepad-resilience release*: a redirected USB **interrupt-IN endpoint** (e.g. an Xbox controller's input pipe) no longer goes dead when the client fails a single read — a channel-still-open transient failure is now treated as an empty poll so the macOS driver keeps polling instead of stalling the pipe (scoped to interrupt endpoints; mass-storage bulk is unaffected). Builds on v0.8.30 (the *webcam release* — a **bulk USB webcam redirected over FreeRDP streams live video** via a bulk-IN read-ahead engine; `--enable-usb-redirection`, entitled build; a first for any open-source RDP *server*).
