...
v0 — daily-driver usable on a trusted LAN, and usable **over the internet** (VPN / ZeroTier / high-latency links, including mobile). **Latest release: [v0.8.38](https://github.com/clintcan/macrdp/releases/latest)** — *the A/V resync hotkey*: a new **`Ctrl+Alt+Shift+R`** recovers a session gone stale after a long idle — repaints a blanked screen (forced keyframe) and re-syncs drifted audio, without disconnecting (handy for mstsc after hours idle). The default runtime path is unchanged. Builds on v0.8.37 (*live resize* via MS-RDPEDISP + the *USB webcam stall watchdog*), v0.8.36 (the *fork-workers removal* + camera-redirection Phase-0 groundwork), v0.8.35 (the *USB read-ahead gate fix*), and v0.8.34 (the *gamepad-input fix*).
