...
v0 — daily-driver usable on a trusted LAN. **Latest release: [v0.8.9](https://github.com/clintcan/macrdp/releases/latest)** — docs/housekeeping: a USB-redirection entitlement-request draft ([`docs/entitlement-request.md`](docs/entitlement-request.md)), account-identifier scrub from the docs, and a Linux-stub lint cleanup. The generic-USB-redirection [feasibility writeup](docs/usb-redirection-feasibility.md) landed in v0.8.8, the GUI smart-card-installer self-kill fix in v0.8.6, and the visual [app-switcher HUD](#cli) (`--app-switcher-hud`) in v0.8.3. See [CLAUDE.md](CLAUDE.md) for what's wired up, what isn't, and known quirks.
