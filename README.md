...
v0 — daily-driver usable on a trusted LAN, and usable **over the internet** (VPN / ZeroTier / high-latency links, including mobile). **Latest release: [v0.8.33](https://github.com/clintcan/macrdp/releases/latest)** — the *audit-forwarding release*: an opt-in **structured JSON audit stream** (`--audit-file`) so a standard collector (Vector / Fluent Bit / rsyslog) can forward macrdp's connection audit events to a **SIEM/SOC**, a new explicit **`auth` login-verdict event** (success / did-not-complete), log-injection hardening of the audit `reason`, an interpretation guide + collector configs, and an end-to-end FreeRDP CI test of the audit writes. All opt-in and default-off — no change to the default runtime path. Builds on v0.8.32 (the *security-hardening release* — fuzzed the network-facing decoders, `--max-client-size` cap, bounded the smart-card bridge, scheduled `cargo-deny`).
