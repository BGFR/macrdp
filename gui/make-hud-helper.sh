#!/bin/bash
# Build the macrdp app-switcher HUD helper (`macrdphud`): swift build the SwiftPM
...
# (packaging/make-app.sh does the embed) and that macrdp spawns to draw the
# on-screen switcher overlay (--app-switcher-hud).
#
...
#   gui/make-hud-helper.sh                                  # build + sign -> gui/.build/release/macrdphud
#   INSTALL_INTO=/Applications/macrdp.app gui/make-hud-helper.sh   # also copy into that app
#
# Env overrides:
...
echo "==> swift build -c release --product macrdphud (identity: $IDENTITY)"
( cd "$GUI_DIR" && swift build -c release --product macrdphud )
BIN="$GUI_DIR/.build/release/macrdphud"
...
if [ -n "${INSTALL_INTO:-}" ]; then
    DEST="$INSTALL_INTO/Contents/Resources/macrdphud"
    cp "$BIN" "$DEST"
    chmod +x "$DEST"
