    ...
    pub async fn read_by_hint(&mut self, hint: &dyn PduHint) -> io::Result<Bytes> {
        loop {
            match hint.find_size(self.peek()).map_err(io::Error::other)? {
                Some((matched, length)) => {
                    // (macrdp vendored divergence — PR #1556, commit c541d09e) Guard against
                    // a zero-length UNMATCHED PDU. Skipping it (the `else` below) consumes
                    // nothing via `read_exact(0)` and performs no I/O, so the loop would spin
                    // forever at 100% CPU — never yielding to the runtime, never observing EOF,
                    // and starving the executor (an acceptor connection loop → the whole server
                    // wedges). Reachable UNAUTH, pre-TLS, from a single malformed 2-byte
                    // fast-path frame. Fail instead of hanging rather than relying on every
                    // `PduHint` to avoid the degenerate case. (Complements upstream #1515's
                    // find_size hardening; drop this vendor dir when the pin bumps past both.)
                    if length == 0 && !matched {
