...
pub trait CliprdrBackend: AsAny + core::fmt::Debug + Send {
    ...
    /// Called by [`crate::Cliprdr`] when the remote responds to a FormatList we sent
    /// (i.e. an outbound advertise of our own clipboard contents).
    ///
    /// `ok = true` means the remote accepted the list (`CB_RESPONSE_OK`); `ok = false`
    /// means it rejected it (`CB_RESPONSE_FAIL`), and [`crate::Cliprdr`] has already
    /// cleared `local_file_list`/`local_file_list_format_id` as required by
    /// MS-RDPECLIP 3.1.5.2.4.
    ///
    /// Backends can use this to decide whether a retry is needed (Fail) and, just as
    /// importantly, to STOP re-advertising once an Ok is seen — a later blind
    /// re-advertise that gets rejected would wipe an already-accepted state.
    ///
