...
pub struct Cliprdr<R: Role> {
    ...
    /// FormatList we sent alongside FileGroupDescriptorW from
    /// `initiate_file_copy`. Tracked so we can recognize a FormatDataRequest
    /// for it and respond inline with `DROPEFFECT_COPY` (0x00000001). This
    /// is what causes Windows Explorer to engage its shell file-copy
    /// machinery (with the native "Copying… items" progress dialog) for
    /// the paste — without it Explorer falls back to a plain synchronous
    /// read with no progress UI, just a busy cursor.
...
impl<R: Role> Cliprdr<R> {
    ...
    pub fn initiate_file_copy(&mut self, files: Vec<FileDescriptor>) -> PduResult<CliprdrSvcMessages<R>> {
        ...
        // Pick a distinct private-range ID for "Preferred DropEffect". The
        // value doesn't matter on the wire (the remote keys off the format
        // *name*); it just has to be locally unique so we can tell which
        // FormatDataRequest is which.
        ...
        // Windows Explorer needs the latter to engage its shell file-copy
        // machinery (with the native progress dialog) on paste — without it,
        // Explorer falls back to a plain synchronous IStream read with no
        // progress UI (just a busy cursor).
        let formats = vec![
    ...
    fn process(&mut self, payload: &[u8]) -> PduResult<Vec<SvcMessage>> {
        ...
        match pdu {
            ...
            ClipboardPdu::FormatDataRequest(request) => {
                ...
                // 4-byte little-endian). This is what we always mean by a
                // server-to-client file copy via FILE_LIST — there is no
                // "move" semantic here — and answering inline keeps the
