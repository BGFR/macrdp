...
impl ClipboardFormatName {
    ...
    /// (1 = DROPEFFECT_COPY, 2 = DROPEFFECT_MOVE). When advertised alongside
    /// `FILE_LIST` (FileGroupDescriptorW), Windows Explorer engages its
    /// shell file-copy machinery on paste (with a native "Copying… items"
