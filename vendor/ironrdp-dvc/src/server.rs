...
impl SvcProcessor for DrdynvcServer {
    ...
    fn process(&mut self, payload: &[u8]) -> PduResult<Vec<SvcMessage>> {
        ...
        match pdu {
            ...
            // (macrdp divergence) The client's Soft-Sync Response. Upstream had no
            // arm for it and the catch-all in DrdynvcClientPdu::decode errored,
            // tearing down the connection. We just acknowledge it — the client has
            // switched the listed tunnels; routing DVC data onto the UDP tunnel is
            // the multitransport layer's job (macrdp), not the drdynvc server's.
            DrdynvcClientPdu::SoftSyncResponse(resp_pdu) => {
                debug!("Got DVC Soft-Sync Response PDU: {resp_pdu:?}");
            }
        }
